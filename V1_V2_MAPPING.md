...
| `k_lower_rt = 0`, `k_upper_rt = 10`, `k_alpha_rt = 1`, `k_beta_rt = 20` | EMG tail: `k ~ 0 + Beta(1,20)·10`, `λ = 1/(kσ)` (`simulate_frame_distributions_emg.py:279-288`) | none — v2's elution profile is a pure Gaussian (`render.rs:1219, 1232`, `gauss_frac`) | **(c)** | Mean `k` = 10/21 = 0.476, so the mean exponential tail constant is `τ = kσ ≈ 0.72 s`. v2 has no tailing at all. This is a *shape* difference a search engine's peak-shape scoring can see. |
...
| `noise_frame_abundance = false`, `noise_scan_abundance = false` | per-frame / per-scan multiplicative jitter `signal·(1+U(0, nl))`, `nl ~ U(0,2)`, renormalised (`utility.py:57-61`) | none | **(c)** | Off in the reference config. v2's elution and mobility weights are strictly deterministic; there is no way to add this jitter. |
